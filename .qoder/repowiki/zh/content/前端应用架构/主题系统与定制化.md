...
## 更新摘要
**变更内容**
- 更新了设计系统令牌的采用说明，详细解释了语义化CSS变量的使用
- 新增了语义化变量命名规范和最佳实践
- 增强了主题一致性与可维护性的说明
- 更新了主题开发指南中的变量使用建议
...
## 简介
本文件系统性阐述 Memos 前端的主题系统与定制化能力，涵盖语义化CSS变量体系、Tailwind CSS 集成、动态主题切换流程、内置主题特性与适用场景、用户自定义主题机制、主题持久化与回退策略、以及最佳实践与兼容性建议。主题系统现已全面采用设计系统令牌，通过语义化CSS变量（如 `--background`、`--foreground`、`--card`、`--popover` 等）替代硬编码颜色值，显著提升了主题一致性和可维护性。

## 项目结构
主题系统主要由以下部分组成：
- **语义化CSS变量与主题样式**：位于 themes 目录，包含默认浅色、深色、午夜、纸张、白墙等主题的 CSS 变量定义与 Tailwind @theme 映射
...
graph TB
subgraph "主题样式"
D["default.css<br/>语义化CSS变量"]
DD["default-dark.css<br/>语义化CSS变量"]
M["midnight.css<br/>语义化CSS变量"]
P["paper.css<br/>语义化CSS变量"]
W["whitewall.css<br/>语义化CSS变量"]
end
subgraph "运行时逻辑"
...
## 架构总览
主题系统采用"语义化CSS变量 + Tailwind 类名 + 运行时注入"的组合方案：
- **语义化CSS变量层**：各主题以 OKLCH 定义语义化颜色变量，覆盖背景、前景、卡片、弹出层、输入框、边框、强调色、破坏性操作、图表色板与侧边栏等
...
## 结论
该主题系统以语义化CSS变量为核心，结合 Tailwind 的类名驱动暗色模式与运行时注入机制，实现了高性能、可扩展且易于维护的主题体系。通过明确的回退链、持久化策略与无障碍设计，既满足了多场景使用需求，也为后续扩展自定义主题提供了清晰路径。新的设计系统令牌采用显著提升了主题的一致性和可维护性。
...
- **跨浏览器兼容**
  ...
  - 在 SSR 环境中优雅降级
- **设计系统令牌使用指南**
  - 使用 `var(--background)` 替代 `#ffffff`
  - 使用 `var(--primary)` 和 `var(--primary-foreground)` 确保正确的对比度
  - 通过 `@theme` 映射确保 Tailwind 工具类与 CSS 变量一致
  - 避免直接修改 `--color-*` 变量，应通过语义化变量进行定制