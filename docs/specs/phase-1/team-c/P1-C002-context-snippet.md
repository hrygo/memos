# P1-C002: 上下文智能摘录

> **状态**: 🔄 进行中  
> **优先级**: P1 (重要)  
> **投入**: 2 人天  
> **负责团队**: 团队 C  
> **Sprint**: Sprint 1-2

---

## 1. 目标与背景

### 1.1 核心目标

基于搜索高亮功能，智能提取匹配位置的上下文摘录，展示最相关的内容片段。

### 1.2 用户价值

- 无需点击即可预览关键内容
- 快速判断搜索结果相关性

### 1.3 技术价值

- 扩展高亮服务能力
- 为相关推荐提供复用

---

## 2. 依赖关系

### 2.1 前置依赖

- [ ] P1-C001: 搜索结果高亮（核心依赖）

### 2.2 并行依赖

- 无

### 2.3 后续依赖

- P1-C003: 相关笔记推荐

---

## 3. 功能设计

### 3.1 架构图

```
┌─────────────────────────────────────────────────────────────────┐
│                    智能摘录逻辑                                   │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  输入: 内容 + 匹配位置列表 + contextChars                        │
│      │                                                          │
│      ▼                                                          │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │  Step 1: 选择最佳匹配点                                    │   │
│  │                                                          │   │
│  │  • 优先选择第一个匹配（最相关）                           │   │
│  │  • 如有多个匹配，选择距离开头最近的                       │   │
│  │  • 如匹配在中间，居中展示                                 │   │
│  └────────────────────────┬────────────────────────────────┘   │
│                           │                                     │
│                           ▼                                     │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │  Step 2: 提取上下文窗口                                    │   │
│  │                                                          │   │
│  │  • 以匹配点为中心                                        │   │
│  │  • 前后各取 contextChars 字符                            │   │
│  │  • 智能调整到词边界（避免截断词语）                       │   │
│  └────────────────────────┬────────────────────────────────┘   │
│                           │                                     │
│                           ▼                                     │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │  Step 3: 格式化输出                                        │   │
│  │                                                          │   │
│  │  • 开头不是原文开头 → 添加 "..."                         │   │
│  │  • 结尾不是原文结尾 → 添加 "..."                         │   │
│  │  • 调整高亮位置偏移                                      │   │
│  └─────────────────────────────────────────────────────────┘   │
│                                                                 │
│  输出: 摘录文本 + 调整后的高亮位置                               │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

### 3.2 核心流程

1. **选点**: 选择最佳匹配点
2. **提取**: 以匹配点为中心提取上下文
3. **调整**: 调整到词边界
4. **格式化**: 添加省略号和位置偏移

### 3.3 关键决策

| 决策点 | 方案 A | 方案 B | 选择 | 理由 |
|:---|:---|:---|:---:|:---|
| 摘录长度 | 固定长度 | 可配置 | B | 适应不同场景 |
| 词边界调整 | 精确分词 | 简单规则 | B | 性能优先 |

---

## 4. 技术实现

### 4.1 关键代码路径

| 文件路径 | 职责 |
|:---|:---|
| `server/service/memo/snippet.go` | 摘录提取 |

### 4.2 核心实现

```go
// server/service/memo/snippet.go

type SnippetExtractor struct {
    defaultContextChars int
}

func NewSnippetExtractor() *SnippetExtractor {
    return &SnippetExtractor{
        defaultContextChars: 50,
    }
}

// ExtractSnippet 提取智能摘录
func (e *SnippetExtractor) ExtractSnippet(
    content string,
    matches []Highlight,
    contextChars int,
) (string, []Highlight) {
    if contextChars == 0 {
        contextChars = e.defaultContextChars
    }
    
    // 无匹配，返回开头
    if len(matches) == 0 {
        snippet := e.truncate(content, contextChars*2)
        return snippet, nil
    }
    
    // 取第一个匹配点为中心
    center := matches[0].Start
    
    // 计算窗口
    start := max(0, center-contextChars)
    end := min(len(content), center+contextChars)
    
    // 调整到词边界
    start = e.adjustToWordBoundary(content, start, false)
    end = e.adjustToWordBoundary(content, end, true)
    
    snippet := content[start:end]
    
    // 添加省略号
    prefix := ""
    suffix := ""
    if start > 0 {
        prefix = "..."
    }
    if end < len(content) {
        suffix = "..."
    }
    snippet = prefix + snippet + suffix
    
    // 调整高亮位置偏移
    adjustedMatches := e.adjustMatchPositions(matches, start, len(prefix))
    
    return snippet, adjustedMatches
}

// adjustToWordBoundary 调整到词边界
func (e *SnippetExtractor) adjustToWordBoundary(content string, pos int, isEnd bool) int {
    if pos <= 0 || pos >= len(content) {
        return pos
    }
    
    // 简单规则：找到最近的空格或标点
    separators := []rune{' ', '\n', '。', '，', '、', '；', '：'}
    
    if isEnd {
        // 向后找
        for i := pos; i < len(content) && i < pos+10; i++ {
            for _, sep := range separators {
                if rune(content[i]) == sep {
                    return i
                }
            }
        }
    } else {
        // 向前找
        for i := pos; i > 0 && i > pos-10; i-- {
            for _, sep := range separators {
                if rune(content[i]) == sep {
                    return i + 1
                }
            }
        }
    }
    
    return pos
}

// adjustMatchPositions 调整匹配位置
func (e *SnippetExtractor) adjustMatchPositions(
    matches []Highlight,
    windowStart int,
    prefixLen int,
) []Highlight {
    var adjusted []Highlight
    
    for _, m := range matches {
        // 只保留在窗口内的匹配
        if m.Start >= windowStart {
            adjusted = append(adjusted, Highlight{
                Start:       m.Start - windowStart + prefixLen,
                End:         m.End - windowStart + prefixLen,
                MatchedText: m.MatchedText,
            })
        }
    }
    
    return adjusted
}
```

---

## 5. 交付物清单

### 5.1 代码文件

- [ ] `server/service/memo/snippet.go` - 摘录提取器

### 5.2 测试文件

- [ ] `server/service/memo/snippet_test.go`

---

## 6. 测试验收

### 6.1 功能测试

| 场景 | 输入 | 预期输出 |
|:---|:---|:---|
| 开头匹配 | 匹配在前10字符 | 从头开始摘录 |
| 中间匹配 | 匹配在中间 | 居中摘录，前后有省略号 |
| 结尾匹配 | 匹配在最后 | 到结尾摘录 |
| 多匹配 | 多个匹配点 | 选择第一个 |

### 6.2 性能验收

| 指标 | 目标值 | 测试方法 |
|:---|:---|:---|
| 提取延迟 | < 10ms | 单元测试 |

---

## 7. ROI 分析

| 维度 | 值 |
|:---|:---|
| 开发投入 | 2 人天 |
| 预期收益 | 搜索预览体验提升 |
| 风险评估 | 低 |
| 回报周期 | Phase 1 结束 |

---

## 8. 实施计划

### 8.1 时间表

| 阶段 | 时间 | 任务 |
|:---|:---|:---|
| Day 1 | 1人天 | 提取逻辑实现 |
| Day 2 | 1人天 | 测试 + 边界处理 |

### 8.2 检查点

- [ ] Day 1: 核心逻辑完成
- [ ] Day 2: 边界case处理

---

## 附录

### A. 参考资料

- [笔记增强路线图](../../research/memo-roadmap.md)

### B. 变更记录

| 日期 | 版本 | 变更内容 | 作者 |
|:---|:---|:---|:---|
| 2026-01-27 | v1.0 | 初始版本 | - |
