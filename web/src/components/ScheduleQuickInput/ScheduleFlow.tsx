...
export function ScheduleFlow({ currentStep, conversation, scheduleData, onSubmit, onConfirm, onCancel, className }: ScheduleFlowProps) {
  ...
  return (
    <div className={cn("flex flex-col", className)}>
      {/* Conversation Area */}
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-3 space-y-3 max-h-[300px] min-h-[120px]">
        ...
        <div className="px-4 py-3 border-t border-border/50 bg-muted/30">
          ...
          <div className="flex items-center justify-between mt-3">
            ...
            <div className="flex gap-2">
              ...
              <Button size="sm" onClick={onConfirm} className="h-8">
      ...
      {currentStep !== "confirmation" && (
        <div className="flex items-center gap-2 px-4 py-3 border-t border-border/50">
          ...
          />
          <Button size="sm" onClick={handleSubmit} disabled={!input.trim()} className="h-9 shrink-0">
            {t("ai.send-shortcut") as string}
          </Button>
          {conversation.length > 0 && (
            <Button variant="ghost" size="sm" onClick={onCancel} className="h-9 px-2 shrink-0">
              <X className="h-4 w-4" />
            </Button>
          )}
        </div>
...
export function CompactScheduleFlow({ scheduleData, onEdit, className }: CompactFlowProps) {
  ...
  return (
    <div className={cn("flex items-center gap-3 p-2 rounded-lg bg-primary/5", className)}>
      <div className="flex-1 min-w-0">
        <div className="font-medium text-sm truncate">{scheduleData.title || (t("schedule.quick-input.default-title") as string)}</div>
      ...
      <Button size="sm" variant="outline" onClick={onEdit} className="h-7 shrink-0">
