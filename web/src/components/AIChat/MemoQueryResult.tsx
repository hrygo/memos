...
export function MemoQueryResult({ result, className }: MemoQueryResultProps) {
  ...
  return (
    <div className={cn(
      "rounded-lg bg-zinc-50 dark:bg-zinc-800/50 border border-zinc-200 dark:border-zinc-700 overflow-hidden",
      className
    )}>
      {/* Header */}
...
function MemoQueryResultItem({ memo, rank }: MemoQueryResultItemProps) {
  ...
  return (
    <Link
      to={`/memo/${memo.uid}`}
      className="block px-4 py-3 hover:bg-zinc-100 dark:hover:bg-zinc-700/50 transition-colors"
    >
